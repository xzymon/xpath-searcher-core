...
public class NodeRepresentation {
	...
	private NodeRepresentation(HalfElementRepresentation openingSlice, NodeRepresentation parent, String name)
			throws IsNotOpeningTagException {
		this.name = name;
		this.parent = parent;
	...
	public static NodeRepresentation buildStructure(List<HalfElementRepresentation> slices, char[] chars)
			throws BuildingDOMException {
		...
		if (slices != null && slices.size() != 0) {
			...
			while (slIt.hasNext()) {
				...
				if (!curSlice.isOther() && !curSlice.isRaw()) {
					extractedName = NodeRepresentation.extractName(chars, curSlice.getNameStartPosition(),
							curSlice.getNameEndPosition());
					if (curNode == null) {
						// to musi być root
						logger.info(String.format("Attempt to create root node from SR at %1$d, with name: \"%2$s\"",
								sloop, extractedName));
						curNode = new NodeRepresentation(curSlice, null, extractedName);
						result = curNode;
						parentNode = result;
						logger.info(String.format("Root node created from SR at %1$d, with name: \"%2$s\"", sloop,
								extractedName));
					} else {
						...
						if (curSlice.isClosing()) {
							if (extractedName.equals(parentNode.getName())) {
								...
								logger.info(String.format("Closing slice found at %1$d, with name: \"%2$s\"", sloop,
										extractedName));
								logger.info(String.format("Moving up, to parent node with name: \"%1$s\"",
										previousParentName));
							} else {
						...
						if (curSlice.isSelfClosing()) {
							...
							logger.info(String.format("Self-closing slice found at %1$d, with name: \"%2$s\"", sloop,
									extractedName));
							continue;
						}
						if (curSlice.isOpening()) {
							...
							parentNode.addChild(curNode);
							parentNode = curNode;
							logger.info(String.format("New opening slice found at %1$d, with name: \"%2$s\"", sloop,
									extractedName));
							continue;
						}
					}
